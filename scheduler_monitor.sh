...
get_schedule_exec_details()(
    ...
    local JOB_ID=$(echo $LATEST_JOB | jq -r '.id')
    local JOB_STATUS=$(echo $LATEST_JOB | jq -r '.status')
    local EXECUTION_ID=$(echo $LATEST_JOB | jq -r '.execution.order')
    local JOB_ORDER=$(echo $LATEST_JOB | jq  -r '.order')
    local QUEUED_AT=$(echo $LATEST_JOB | jq -r '.queuedAt')
    local STARTED_AT=$(echo $LATEST_JOB | jq -r  '.startTime')
    local STOPPED_AT=$(echo $LATEST_JOB | jq -r '.stopTime')
